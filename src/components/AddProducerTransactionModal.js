  ...
  const fetchProducers = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (
				...
				parseInt(formData.get('box_qtt')) > (getBoxAxios.qtt_total - getBoxAxios.box_qtt)
			) {
			...
			if (formData.get('transaction_type') === 'lend' && formData.get('box_type') === 'premium') {
				getProducerAxios.box_premium += parseInt(formData.get('box_qtt'));
			} else if (formData.get('transaction_type') === 'devolution' && formData.get('box_type') === 'premium') {
				getProducerAxios.box_premium -= parseInt(formData.get('box_qtt'));
			}

			if (formData.get('transaction_type') === 'lend' && formData.get('box_type') === 'common') {
				getProducerAxios.box_common += parseInt(formData.get('box_qtt'));
			}
			else if (formData.get('transaction_type') === 'devolution' && formData.get('box_type') === 'common') {
				getProducerAxios.box_common -= parseInt(formData.get('box_qtt'));
			}
	...
	return(
		<div className="container">
			...
			<Modal className="container modals" 
				{...props}
				size="mg"
							...
							<Col>
								<FormGroup controlId="producer">
									...
									<Form.Select name="producer" required defaultValue="">
										<option value="" disabled>Choose a producer</option>
						...
						<Row className='row-css'>
							<Col sm={6}>
								<Form.Group controlId="box_qtt">
									...
									<Form.Control type="number" name="box_qtt" required placeholder="" min="0"/>
								</Form.Group>
								{showLendErrorMessage && (
									<p className="error-message">more box than there is in the storehouse</p>
								)}
								{showDevErrorMessage && (
									<p className="error-message">more box than there is with producers</p>
								)}
							</Col>
							...
							<Col sm={6}>
								<Form.Group controlId="price">
									...
									<Form.Control type="number" name="price" required placeholder="" min="0"/>
								</Form.Group>
							</Col>
						</Row>
